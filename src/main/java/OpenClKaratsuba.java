...
public class OpenClKaratsuba {
    private static String programSource =
            "__kernel void "+
                    "sampleKernel(__global char *a,"+
    ...
    public void execute(BigInteger a,int b[],int prod[],int pos){
        int[] q = new int[]{pos};
        Pointer srcQ = Pointer.to(q);
        //char[] aa = new char[]{0,1,2};
        byte[] bytes = new byte[1];
        try {
            bytes = "123".toString().getBytes("UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        //System.out.println("sizeof " + (Sizeof.cl_char * (aa.length + 1)));
        Pointer srcA = Pointer.to(bytes);
        ...
        memObjects[0] = clCreateBuffer(context,
                ...
                Sizeof.cl_char * bytes.length, srcA, null);
